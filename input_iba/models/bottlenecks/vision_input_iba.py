...
class VisionInputIBA(BaseInputIBA):
    ...
    def init_alpha_and_kernel(self):
        ...
        self.alpha = nn.Parameter(
            torch.full(shape, self.initial_alpha),     #, device=self.device #CHANGE CLUSTER
            requires_grad=True)
    ...
    def do_restrict_info(self, x, alpha):
        ...
        buffer_capacity = self.kl_div(x, self.input_mask, lamb,
                                           self.input_mean, self.input_std)
        
        if self.buffer_capacity is not None:
            buffer_capacity = buffer_capacity.unsqueeze(0)
            self.buffer_capacity = torch.cat((self.buffer_capacity, buffer_capacity.to(self.buffer_capacity.device)))
        else:
        ...
        with self.restrict_flow():
            for i in range(opt_steps):
                ...
                for j in range(batch_size):
                    images.append(self.forward(batch).unsqueeze(0))
                    
                masked_img = torch.cat(images)
