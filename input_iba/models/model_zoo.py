...
def build_classifiers(cfg, default_args=None):
    ...
    assert source in ('torchvision', 'custom', 'timm', 'nlp', 'mil'), \
        f"source should be on of ('custom', 'torchvision', 'timm', 'mil'), " \
        f"but got {source}"
    ...
    if source == 'torchvision':
        ...
        return build_mil_classifiers(cfg)
    else:
        return build_from_cfg(cfg, MODELS, default_args=default_args)
    
    
def build_mil_classifiers(cfg):
