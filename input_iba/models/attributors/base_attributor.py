...
class BaseAttributor(metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.classifier = self.build_classifier(
            classifier, device=self.device, eval=eval_classifier)
        self.layer = layer
    ...
    def train_gan(self, input_tensor, attr_cfg, sample, logger=None):
        default_args = {
            'input_tensor': input_tensor[sample],
            'context': self,
            # TODO check whether to pass capacity or
            #  sigmoid(self.feat_iba.alpha)
            'feat_mask': self.feat_iba.capacity()[sample],
            'device': self.device
        }
    ...
    def make_attribution(self,
        ...
        for i in range(input_tensor.shape[0]):
            gans.append(self.train_gan(
            input_tensor, attr_cfg['gan'], i, logger=logger))
                
        gen_input_mask = torch.cat(gans)
        ...
        gen_input_mask = gen_input_mask.mean([1]).cpu().numpy()
